...
export class OrdersService {
  ...
  async checkout(
  ...
  ): Promise<{ orders: OrderDocument[]; paymentData?: any }> {
    ...
    const existingOrders = await this.orderModel
      .find({ userId: new Types.ObjectId(userId), idempotencyKey })
      .exec();
    ...
    for (const item of cart.items) {
      const restaurantIdStr =
        typeof item.restaurantId === 'object' && (item.restaurantId as any)._id
      ...
      const menuItemIdStr =
        typeof item.menuItemId === 'object' && (item.menuItemId as any)._id
    ...
    for (const [restaurantIdStr, items] of itemsByRestaurant.entries()) {
      const subTotal = items.reduce(
        (acc, current) => acc + current.totalItemPrice,
        0,
      );

      if (subTotal < settings.minOrderValue) {
        throw new BadRequestException(
          `Order from restaurant must be at least ₹${settings.minOrderValue}`,
        );
      }
      ...
      const itemDiscount = items.reduce((acc, current) => {
        const savings =
          current.originalPrice > current.price
            ? (current.originalPrice - current.price) * current.quantity
            : 0;
        return acc + savings;
      }, 0);
      ...
      if (checkoutDto.couponCode) {
        const couponValidation = await this.promotionsService.validateCoupon({
        ...
        if (couponValidation && couponValidation.valid) {
          couponDiscount = Math.round(couponValidation.discountAmount);
          appliedCouponCode = checkoutDto.couponCode.toUpperCase();
        }
      }

      const discountAmount = itemDiscount + couponDiscount;
      const totalAmount = Math.max(
        0,
        subTotal + staticTax + staticDeliveryFee + packagingFee - couponDiscount,
      );
      ...
      if (appliedCouponCode) {
        await this.promotionsService.recordPromotionUsage(appliedCouponCode, userId);
      }
    }
    ...
    for (const order of createdOrders) {
      ...
      if (restaurant && restaurant.ownerId) {
        ...
        if (owner.firebaseUid) {
          ...
          this.socketsGateway.emitNewOrder(
            owner.firebaseUid,
  ...
  async initiatePayment(
    ...
    if (!cart || cart.items.length === 0) {
      throw new BadRequestException(
        'Cannot initiate payment with an empty cart',
      );
    }
    ...
    for (const [restaurantIdStr, items] of itemsByRestaurant.entries()) {
      ...
      if (subTotal < settings.minOrderValue) {
        throw new BadRequestException(
          `Order from restaurant must be at least ₹${settings.minOrderValue / 100}`,
        );
      }
      ...
      const itemDiscount = items.reduce((acc, i) => {
        return (
          acc +
      ...
      const totalAmount = Math.max(
        0,
        subTotal + staticTax + deliveryFee + packagingFee - couponDiscount,
      );
  ...
  async confirmPayment(
    ...
    if (transaction.status === 'SUCCESS') {
      ...
      const existingOrders = await this.orderModel
        .find({ idempotencyKey: transaction.metadata?.idempotencyKey })
        .exec();
      return { orders: existingOrders };
    }

    const { orderSnapshots } = transaction.metadata;
    if (!orderSnapshots?.length)
      throw new BadRequestException('No order data found in payment session');
    ...
    for (const snap of orderSnapshots) {
      ...
      if (snap.couponCode) {
        await this.promotionsService.recordPromotionUsage(snap.couponCode, userId);
      }
    }
  ...
  async getUserOrders(
    ...
    const [data, totalItems] = await Promise.all([
      this.orderModel
        ...
        .populate(
          'restaurantId',
          'name logo coverImages rating address location',
        )
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
  ...
  async getOrderById(id: string, currentUser?: any): Promise<Order> {
    const order = await this.orderModel
      ...
      .populate(
        'restaurantId',
        'name logo coverImages rating address location ownerId',
      )
      .populate('userId', 'name phone email')
      .populate({
        path: 'driverId',
    ...
    if (currentUser && currentUser.role !== 'admin') {
      ...
      if (order.driverId) {
        try {
          ...
          if (
            driver &&
            order.driverId._id.toString() === driver._id.toString()
          ) {
            isDriverObj = true;
          }
        } catch (e) {}
  ...
  async updateOrderStatus(
    ...
    if (updateOrderStatusDto.status === OrderStatus.DELIVERED) {
      throw new BadRequestException(
        'Status "DELIVERED" cannot be set through this endpoint. Please use the dedicated delivery endpoint.',
      );
    }

    if (currentUser && currentUser.role !== 'admin') {
      const orderObj = await this.orderModel
        .findById(id)
      ...
      const isCustomer =
        orderObj.userId.toString() === currentUser._id.toString();
      const isVendor =
        (orderObj.restaurantId as any).ownerId.toString() ===
        currentUser._id.toString();
      ...
      if (orderObj.driverId) {
        try {
          ...
          if (
            driver &&
            orderObj.driverId.toString() === driver._id.toString()
          ) {
            isDriverObj = true;
          }
        } catch (e) {}
    ...
    if (restaurant && restaurant.ownerId) {
      const owner = restaurant.ownerId as any;
      if (owner.firebaseUid) {
        this.socketsGateway.emitOrderStatusToVendor(
          owner.firebaseUid,
          id,
    ...
    if (order.driverId) {
      try {
        ...
        if (driverUser && driverUser.firebaseUid) {
          this.socketsGateway.emitOrderStatusToDriver(
            driverUser.firebaseUid,
            id,
            updateOrderStatusDto.status,
          );
        }
  ...
  async assignDriver(
    ...
    if (currentUser && currentUser.role !== 'admin') {
      ...
      if (!isVendor && !isTargetDriver) {
        throw new ForbiddenException(
          'You are not authorized to assign drivers to this order',
        );
      }
    }
    ...
    if (!driver.isAvailable) {
      throw new BadRequestException(
        `Driver with ID ${driverId} is currently not available.`,
      );
    }
  ...
  async markDelivered(id: string, driverId?: string): Promise<Order> {
    ...
    if (
      driverId &&
      (!order.driverId || order.driverId.toString() !== driverId)
    ) {
      throw new BadRequestException(
        'You are not authorized to mark this order as delivered. Is it assigned to you?',
      );
    }
    ...
    if (
      ...
      throw new BadRequestException(
        `Cannot deliver an order in ${order.status} status.`,
      );
    }
    ...
    await this.runInTransaction(async (session) => {
      ...
      if (durationMinutes > 0 && durationMinutes < 180) {
        ...
        if (
          restaurant &&
          restaurant.deliveryCount &&
          restaurant.deliveryCount > 0
        ) {
          const avgMinutes = Math.round(
            (restaurant.totalDeliveryTimeMinutes ?? 0) /
              restaurant.deliveryCount,
          );
          await this.restaurantModel
            .findByIdAndUpdate(
      ...
      await this.usersService.incrementUserStats(
        this.extractId(order.userId),
        order.totalAmount,
      );
    });
  ...
  async autoAssignDriver(
    id: string,
    maxDistanceOrUser: number | any = 10000,
    currentUser?: any,
  ): Promise<{ order: Order; driverId?: string; message: string }> {
    ...
    if (resolvedCurrentUser && resolvedCurrentUser.role !== 'admin') {
      ...
      if (
        (orderObj.restaurantId as any).ownerId.toString() !==
        resolvedCurrentUser._id.toString()
      ) {
        throw new ForbiddenException(
          'You are not authorized to auto-assign a driver to this order',
        );
      }
    }
    ...
    const restaurant = await this.restaurantModel
      ...
      .exec();
    if (!restaurant)
      throw new NotFoundException('Restaurant not found for this order.');
    ...
    if (
      ...
      const nearbyDrivers = await this.driversService.findNearbyAvailable(
        lng,
        lat,
        resolvedMaxDistance,
      );
    ...
    if (!matchedDriver) {
      return { order, message: 'No available drivers found in vicinity.' };
    }

    order.driverId = matchedDriver._id;
    order.status = OrderStatus.PREPARING;
