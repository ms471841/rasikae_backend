...
export class InvoicesService {
  async generateInvoicePdf(order: Order): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      doc.fontSize(10).text(`Order ID: #${(order as any)._id.toString().slice(-6).toUpperCase()}`, 0, 75, { align: 'right' });
      doc.text(`Date: ${order.createdAt.toLocaleDateString()}`, 0, 90, { align: 'right' });

      doc.moveTo(50, 110).lineTo(550, 110).stroke();

      // 2. Billing Details
      doc.fontSize(12).font('Helvetica-Bold').text('Bill To:', 50, 130);
      doc.font('Helvetica').fontSize(10).text((order.userId as any)?.name || 'Valued Customer', 50, 145);
      const addr = order.deliveryAddress;
      doc.text(`${addr?.street || ''}, ${addr?.city || ''}`, 50, 160, { width: 200 });

      doc.fontSize(12).font('Helvetica-Bold').text('Restaurant:', 350, 130);
      doc.font('Helvetica').fontSize(10).text((order.restaurantId as any)?.name || 'Partner Restaurant', 350, 145);
      doc.text((order.restaurantId as any)?.address || '', 350, 160, { width: 200 });
      ...
      doc.fontSize(14).font('Helvetica-Bold').text('Grand Total:', rightColX, y);
