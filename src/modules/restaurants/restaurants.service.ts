...
export class RestaurantsService {
  ...
  async create(
    ...
    await this.cacheService.delByPattern('restaurants:*');

    // 2. Auto initiate wallet ledger
    await this.walletsService.initializeRestaurantWallet(
  ...
  async findAll(
    ...
    if (filters.isPublished !== undefined)
      baseMatch.isPublished = filters.isPublished;
    ...
    if (lat !== undefined && lng !== undefined && !isNaN(lat) && !isNaN(lng)) {
      const effectiveMaxDistance = filters.nearAndFast
        ? 5000
        : (filters.maxDistance || 1000000) * 1000;
    ...
    const totalItems = await this.restaurantModel
  ...
  async getHomeFeed(
    ...
    const recommendedPromise = getCarouselData(
      { rating: -1 },
      { $or: [{ isFeatured: true }, { isFreeDelivery: true }] },
    );
  ...
  async findOne(id: string): Promise<Restaurant> {
    ...
    const cached = await this.cacheService.get<Restaurant>(cacheKey);
    if (cached) {
      return cached;
    }
    ...
    const plainRestaurant = (restaurant as any).toJSON ? (restaurant as any).toJSON() : restaurant;
    await this.cacheService.set(cacheKey, plainRestaurant, 300);
    return restaurant;
  }
  ...
  async update(
    ...
    const payload: any = { ...updateRestaurantDto };
    ...
    if (currentUser && currentUser.role !== 'admin') {
      ...
      // Strip admin-only / system fields
  ...
  async getBankAccount(
    ...
    if (currentUser && currentUser.role !== 'admin') {
      ...
      if (
        ...
        throw new ForbiddenException(
          'You are not authorized to access this restaurant bank account',
        );
      }
    }
    const bankAccount = await this.bankAccountModel
      .findOne({ restaurantId: new mongoose.Types.ObjectId(restaurantId) })
      .exec();
    if (!bankAccount) {
      throw new NotFoundException(
        `Bank account for restaurant ${restaurantId} not found`,
      );
    }
    return bankAccount;
  }
  ...
  async upsertBankAccount(
    ...
    if (currentUser && currentUser.role !== 'admin') {
      ...
      if (
        ...
        throw new ForbiddenException(
          'You are not authorized to manage this restaurant bank account',
        );
      }
    }
    const existing = await this.bankAccountModel
      .findOne({ restaurantId: new mongoose.Types.ObjectId(restaurantId) })
      .exec();
    if (existing) {
